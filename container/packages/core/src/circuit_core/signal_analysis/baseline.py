...
from circuit_core.signal_analysis.ingest import (
    load_data,
    parse_timestamps,
    detect_frequency,
...
def run_baseline(
    ...
    # Safety cap: STL/MSTL on very large series (>10K points) is extremely
    # memory-intensive. If we still have too many rows after resampling,
    # force aggregation to daily.
    MAX_STL_POINTS = 10_000
    if len(df) > MAX_STL_POINTS:
        logger.info(
            "Series too large for STL (%d points > %d cap). "
            "Forcing daily aggregation.",
            len(df), MAX_STL_POINTS,
        )
        df = df[value_cols].resample("1D").sum()
        df = df.loc[df.index.notna()]
        detected_freq = "daily"
        seasonal_periods = get_seasonal_periods(detected_freq)
    ...
    if not skip_report:
        try:
            from circuit_core.signal_analysis.report import generate_baseline_report
...
def _seasonal_summary(seasonal: pd.Series, period_info: dict, freq: str) -> dict:
    ...
    # Persist grouped mean values for embedding extraction
    values = None
    try:
        if label == "weekly" and freq in ("daily", "hourly"):
            values = seasonal.groupby(seasonal.index.dayofweek).mean().reindex(range(7), fill_value=0.0).tolist()
        elif label == "annual" and freq in ("daily", "weekly", "monthly"):
            values = seasonal.groupby(seasonal.index.month).mean().reindex(range(1, 13), fill_value=0.0).tolist()
        elif label == "daily" and freq in ("hourly", "sub_hourly"):
            values = seasonal.groupby(seasonal.index.hour).mean().reindex(range(24), fill_value=0.0).tolist()
    except Exception:
        pass
