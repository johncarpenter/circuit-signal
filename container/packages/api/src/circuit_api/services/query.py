...
# Validated whitelist mapping axis to column name — prevents SQL injection
EMBEDDING_COLS = {
    SearchAxis.SHAPE: "shape_embedding",
    SearchAxis.DEVIATION: "deviation_embedding",
    SearchAxis.TEXT: "text_embedding",
}
...
async def find_similar_by_signal(
    ...
    embedding_col = EMBEDDING_COLS[axis]
...
async def cluster_signals(
    ...
    result = await db.execute(
        text(f"""
            SELECT id::text AS uuid, signal_id, dataset_name, segment,
                   segment_by, text_description,
                   {embedding_col}::text AS embedding
            FROM signals
            WHERE {where_sql}
        """),
        params,
    ...
    coords = pca.fit_transform(X) if X.shape[0] >= 2 else np.zeros((X.shape[0], 2))

    # Attach coordinates to signal data
    for i in range(len(signals_data)):
        signals_data[i]["x"] = round(float(coords[i, 0]), 4)
        signals_data[i]["y"] = round(float(coords[i, 1]), 4)
    ...
    for label, cluster in clusters.items():
        ...
        member_indices = [i for i, l in enumerate(labels) if int(l) == label]
        centroid = kmeans.cluster_centers_[label]
        distances = [np.linalg.norm(X[i] - centroid) for i in member_indices]
        closest = member_indices[int(np.argmin(distances))]
        cluster["representative"] = signals_data[closest]["signal_id"]
